...
public interface WorkDao {
     ...
     * @return
     */
    @Select("select * from t_producework  order by w_id desc limit #{0},#{1};")
    ...
    //    更新生产和未生产
    @Update("  update bjjl set pFlag=2 where id=#{0};")
    int updateProduct(int flag);

    //   书刊生产
    @Update("update hcbj set hflag=2  where hbjjl=#{0};")
    int updateHc(int flag);

    //   画册生产
    @Update("update skbj set s_flag=2  where s_bjjl=#{0};")
    int updateSk(int flag);

    @Update("update t_producework set w_id=#{1}" +
