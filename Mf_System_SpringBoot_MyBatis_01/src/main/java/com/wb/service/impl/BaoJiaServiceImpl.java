...
public class BaoJiaServiceImpl implements BaoJiaService {
    ...
    @Override
    public PageInfo selAll(PageInfo pageInfo,String cName,String pName) {
        int pageSize = pageInfo.getPageSize();
        int pageNumber = pageInfo.getPageNumber();
        int start=(pageNumber-1)*pageSize;
        List<BaoJiaJiLu> baoJiaJiLus = baoJiaDao.selAll(start,pageSize,cName,pName);
        int count=baoJiaDao.selCount(cName,pName);
    ...
    @Override
    public PageInfo selProduceYes(PageInfo pageInfo,String cName,String pName) {
        int count = baoJiaDao.selProduceYesCount(cName,pName);
        int pageSize = pageInfo.getPageSize();
        int pageNumber = pageInfo.getPageNumber();
        int start=(pageNumber-1)*pageSize;
        pageInfo.setPageStart(start);
        List<BaoJiaJiLu> baoJiaJiLus = baoJiaDao.selProduceYes(start, pageSize,cName,pName);
        pageInfo.setList(baoJiaJiLus);
        pageInfo.setPageNumber(pageNumber);
        int total=(count%pageSize==0?count/pageSize:count/pageSize+1);
        pageInfo.setTotal(total);
    ...
    @Override
    public PageInfo selProduceNo(PageInfo pageInfo,String cName,String pName) {
        int count = baoJiaDao.selProduceNoCount(cName,pName);
        int pageSize = pageInfo.getPageSize();
        int pageNumber = pageInfo.getPageNumber();
        int start=(pageNumber-1)*pageSize;
        List<BaoJiaJiLu> baoJiaJiLus = baoJiaDao.selProduceNo(start, pageSize,cName,pName);
        pageInfo.setPageStart(start);
        pageInfo.setList(baoJiaJiLus);
        pageInfo.setPageNumber(pageNumber);
        int total=(count%pageSize==0?count/pageSize:count/pageSize+1);
